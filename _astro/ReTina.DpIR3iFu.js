...
`;function C({sdWGSL:a,materialSdFunctions:e}){function t(s,o){const c=`vec3<f32>(U.material${o}Px, U.material${o}Py, U.material${o}Pz)`,u=`vec3<f32>(U.material${o}Rx, U.material${o}Ry, U.material${o}Rz)`;return`
...
`)}
      ...
      return dist;`;return e=e.replace("// #MAP",n),e}function _({materialLightFunctions:a,sdWGSL:e}){function t(s,o){return s||(s=`
...
`);e=e.replace("// #LIGHT-INDIVIDUAL-MATERIALS",n);function r(s){return`
      ...
      ${a.map((s,o)=>r(o)).join(`
`)}
      return finalColor;`;return e=e.replace("// #LIGHT-MATERIALS-FUNC",i),e}function G(a){let e=I;const t=a.map(r=>r.sdFunc),n=a.map(r=>r.lightFunc);return e=C({sdWGSL:e,materialSdFunctions:t}),e=U({materialSdFunctions:t,sdWGSL:e}),e=_({sdWGSL:e,materialLightFunctions:n}),e}function M({functions:a,rtUniformKeys:e,materialFuncs:t,nTextures:n,usePrevFrameTex:r}){let i=z;if(i=i.replace("UNIFORMS: f32, // #UNIFORMS",e.map(s=>`${s}: f32,`).join(`
...
`);i=i.replace("// #GROUP-1-BINDING-X",o)}if(r&&(i=i.replace("// #GROUP-2-BINDING-X",`
      ...
      }`)),a&&(i=i.replace("// #FUNCTIONS",a)),t.length>0){const s=G(t);i=i.replace("// #RAY_MARCH_FUNCTIONS",s)}return i}function F({main:a,functions:e,rtUniformKeys:t,materialFuncs:n,nTextures:r,usePrevFrameTex:i,useInterlacing:s}){const o=M({functions:e,materialFuncs:n,nTextures:r,rtUniformKeys:t,usePrevFrameTex:i});n.length>0&&!a&&(a=`
  ...
  `;return s&&(c=c.replace("// #INTERLACING",`
    ...
    `)),c}class D{device;buffer;uniform={};uniformBindGroupLayout;uniformBindGroup;constructor(e,t){this.device=e;const n=Object.keys(t);this.buffer=e.createBuffer({size:n.length*4,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),n.forEach((r,i)=>{this.uniform[r]={value:t[r],offSet:i*4},this.set(r,t[r])}),this.uniformBindGroupLayout=e.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.FRAGMENT,sampler:{type:"non-filtering"}}]}),this.uniformBindGroup=this.device.createBindGroup({layout:this.uniformBindGroupLayout,entries:[{binding:0,resource:{buffer:this.getBuffer()}},{binding:1,resource:this.device.createSampler({addressModeU:"repeat",addressModeV:"repeat",magFilter:"nearest",minFilter:"nearest"})}]})}getBuffer(){return this.buffer}getKeysSortedByOffset(){return Object.keys(this.uniform).toSorted((e,t)=>this.uniform[e].offSet-this.uniform[t].offSet)}set(e,t){if(!this.uniform[e])throw new Error(`Uniform ${e} not found`);this.uniform[e].value=t,this.device.queue.writeBuffer(this.buffer,this.uniform[e].offSet,new Float32Array([t]))}get(e){if(!this.uniform[e])throw new Error(`Uniform ${e} not found`);return this.uniform[e].value}}class A{texs;device;presentationFormat;textures;textureEntries;textureBindGroupLayout;constructor(e,t,n){this.texs=n,this.device=e,this.presentationFormat=t,this.textures=this.texs.map(({width:r,height:i})=>this.device.createTexture({size:[r,i,1],format:this.presentationFormat,usage:GPUTextureUsage.COPY_DST|GPUTextureUsage.TEXTURE_BINDING})),this.textureEntries=this.textures.map((r,i)=>({binding:i,resource:r.createView()})),this.textureBindGroupLayout=e.createBindGroupLayout({entries:n.map((r,i)=>({binding:i,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"unfilterable-float"}}))})}setTexture(e,t){const n=this.texs[e].width,r=this.texs[e].height,i=this.textures[e];this.device.queue.writeTexture({texture:i},t,{offset:0,bytesPerRow:n*4,rowsPerImage:r},[n,r,1]),this.textureEntries[e].resource=i.createView()}getTextureEntries(){return this.textureEntries}length(){return this.texs.length}}class N{pingPongBindGroupLayout;bindGroupPingIn;bindGroupPongIn;currentReadTexture;currentReadTextureView;currentWriteTexture;currentWriteTextureView;currentBindGroupIn;offscreenTextureSize;constructor(e,t,n){this.offscreenTextureSize=[e.width,e.height,1];const r=t.createTexture({size:this.offscreenTextureSize,format:n,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),i=t.createTexture({size:this.offscreenTextureSize,format:n,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),s=r.createView(),o=i.createView();this.pingPongBindGroupLayout=t.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"unfilterable-float"}}]}),this.bindGroupPingIn=t.createBindGroup({layout:this.pingPongBindGroupLayout,entries:[{binding:0,resource:r.createView()}]}),this.bindGroupPongIn=t.createBindGroup({layout:this.pingPongBindGroupLayout,entries:[{binding:0,resource:i.createView()}]}),this.currentReadTexture=r,this.currentReadTextureView=s,this.currentWriteTexture=i,this.currentWriteTextureView=o,this.currentBindGroupIn=this.bindGroupPingIn}preSubmit(){}swap(){const e=this.currentReadTexture,t=this.currentReadTextureView,n=this.currentBindGroupIn;this.currentReadTexture=this.currentWriteTexture,this.currentReadTextureView=this.currentWriteTextureView,this.currentBindGroupIn=n===this.bindGroupPingIn?this.bindGroupPongIn:this.bindGroupPingIn,this.currentWriteTexture=e,this.currentWriteTextureView=t}}async function E({device:a,presentationFormat:e,context:t,canvas:n,main:r,materialFuncs:i,functions:s,rtUniform:o,rtTexture:c,usePrevFrameTex:u,useInterlacing:v}){const m=[o.uniformBindGroupLayout,c.textureBindGroupLayout];let l=null;u&&(l=new N(n,a,e),m.push(l.pingPongBindGroupLayout));const f=F({main:r,functions:s,materialFuncs:i,rtUniformKeys:o.getKeysSortedByOffset(),nTextures:c.length(),usePrevFrameTex:u,useInterlacing:v}),g=a.createRenderPipeline({layout:a.createPipelineLayout({bindGroupLayouts:m}),vertex:{module:a.createShaderModule({code:R}),entryPoint:"main"},fragment:{module:a.createShaderModule({code:f}),entryPoint:"main",targets:[{format:e}]},primitive:{topology:"triangle-strip",frontFace:"ccw",stripIndexFormat:"uint32"}});let y=0;const T=performance.now()/1e3;function w(p){const x=performance.now()/1e3-T;o.set("frame",y),o.set("time",x),o.set("aspectRatio",n.width/n.height),o.set("width",n.width),o.set("height",n.height),o.set("camPosX",p.pos.x),o.set("camPosY",p.pos.y),o.set("camPosZ",p.pos.z),o.set("camSphericalR",p.spherical.radius),o.set("camSphericalT",p.spherical.theta),o.set("camSphericalP",p.spherical.phi),o.set("camFov",p.fov)}function b({camera:p}){w(p);const x={colorAttachments:[{view:l?l.currentWriteTextureView:t.getCurrentTexture().createView(),loadOp:"clear",clearValue:{r:0,g:0,b:0,a:1},storeOp:"store"}]},d=a.createCommandEncoder(),h=d.beginRenderPass(x);h.setPipeline(g),h.setBindGroup(0,o.uniformBindGroup);const S=a.createBindGroup({layout:g.getBindGroupLayout(1),entries:c.getTextureEntries()});h.setBindGroup(1,S),l&&h.setBindGroup(2,l.currentBindGroupIn),h.draw(4,1,0,0),h.end(),l&&d.copyTextureToTexture({texture:l.currentWriteTexture},{texture:t.getCurrentTexture()},l.offscreenTextureSize),a.queue.submit([d.finish()]),l&&l.swap(),y++}return b}async function L(){const a=await navigator.gpu.requestAdapter();if(!a)throw new Error("No adapter found");const e=a.features.has("texture-compression-bc");return e||console.warn("shader-f16 not available"),{device:await a.requestDevice({requiredFeatures:e?["shader-f16"]:[]}),presentationFormat:navigator.gpu.getPreferredCanvasFormat()}}class B{setPos;setRot;setColor;constructor({material:e,rt:t,index:n}){e.pos||(e.pos={x:0,y:0,z:0}),e.color||(e.color={r:1,g:1,b:1}),e.rotation||(e.rotation={x:0,y:0,z:0}),e.collisionGroup||(e.collisionGroup=-1);const r=t.registerUniform(`material${n}Px`,e.pos.x),i=t.registerUniform(`material${n}Py`,e.pos.y),s=t.registerUniform(`material${n}Pz`,e.pos.z);this.setPos=f=>{f.x!==void 0&&r(f.x),f.y!==void 0&&i(f.y),f.z!==void 0&&s(f.z)};const o=t.registerUniform(`material${n}Rx`,e.rotation.x),c=t.registerUniform(`material${n}Ry`,e.rotation.y),u=t.registerUniform(`material${n}Rz`,e.rotation.z);this.setRot=f=>{f.x!==void 0&&o(f.x),f.y!==void 0&&c(f.y),f.z!==void 0&&u(f.z)};const v=t.registerUniform(`material${n}Cr`,e.color.r),m=t.registerUniform(`material${n}Cg`,e.color.g),l=t.registerUniform(`material${n}Cb`,e.color.b);this.setColor=f=>{f.r!==void 0&&v(f.r),f.g!==void 0&&m(f.g),f.b!==void 0&&l(f.b)}}}class ${shoot;fps;fpsCounter=0;showFps;cb;constructor({cb:e,shoot:t,fps:n,showFps:r}){this.shoot=t,this.cb=e,this.fps=n,this.showFps=r}startFpsCounter(){const e=document.createElement("div");e.setAttribute("style",` position: absolute;
        ...
        pointer-events: none;`),document.body.appendChild(e),setInterval(()=>{e.innerText=`FPS: ${this.fpsCounter}`,this.fpsCounter=0},1e3)}start(){this.showFps&&this.startFpsCounter();const e=this.cb,t=this.shoot,n=()=>{this.fpsCounter++},r=this.fps;let i=()=>{requestAnimationFrame(i),e(),t(),n()};if(r!==void 0){const s=1e3/r;let o=0;i=c=>{requestAnimationFrame(i);const u=c-o;u>s&&(o=c-u%s,e(),t(),n())}}requestAnimationFrame(i)}}function X(a){const e=document.createElement("canvas");return e.height=a??window.innerHeight,e.width=e.height*(window.innerWidth/window.innerHeight),e.style.width="100vw",e.style.height="100vh",a&&(e.style.imageRendering="pixelated"),document.body.appendChild(e),e}class V{canvas;main;functions;render;initalCustomUniforms={};setUniform;materialFuncs=[];usePrevFrameTex;useInterlacing;setDeviceTexure;texs;fps;showFps;camera={pos:{x:0,y:0,z:0},spherical:{radius:0,theta:0,phi:0},fov:90};constructor({main:e,functions:t,texs:n,usePrevFrameTex:r,useInterlacing:i,fps:s,height:o,showFps:c}={}){this.canvas=X(o),this.main=e,this.functions=t,this.texs=n??[],this.useInterlacing=i,this.usePrevFrameTex=this.useInterlacing||r,this.fps=s,this.showFps=c}registerMaterial(e){if(this.render)throw new Error("Render already built");const t=this.materialFuncs.length,n=new B({rt:this,material:e,index:t});return this.materialFuncs.push({sdFunc:e.sdFunc,lightFunc:e.lightFunc}),n}setTex(e,t){if(!this.setDeviceTexure)throw new Error("Render not built");this.setDeviceTexure(e,t)}registerUniform(e,t){if(this.render)throw new Error("Render already built");return this.initalCustomUniforms[e]=t??0,r=>{if(!this.setUniform)throw new Error("Render not built");this.setUniform(e,r),this.initalCustomUniforms[e]=r}}async build(){const{device:e,presentationFormat:t}=await L(),n=this.canvas.getContext("webgpu");if(!n)throw new Error("No context found");n.configure({device:e,format:t,alphaMode:"opaque",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.COPY_DST});const r=new D(e,{frame:0,time:0,aspectRatio:this.canvas.width/this.canvas.height,width:this.canvas.width,height:this.canvas.height,camPosX:this.camera.pos.x??0,camPosY:this.camera.pos.y??0,camPosZ:this.camera.pos.z??0,camSphericalR:this.camera.spherical.radius??0,camSphericalT:this.camera.spherical.theta??0,camSphericalP:this.camera.spherical.phi??0,camFov:this.camera.fov??90,...this.initalCustomUniforms}),i=new A(e,t,this.texs);this.render=await E({device:e,presentationFormat:t,context:n,canvas:this.canvas,main:this.main,materialFuncs:this.materialFuncs,functions:this.functions,rtUniform:r,rtTexture:i,usePrevFrameTex:this.usePrevFrameTex,useInterlacing:this.useInterlacing}),this.setUniform=(s,o)=>{r.set(s,o)},this.setDeviceTexure=(s,o)=>{i.setTexture(s,o)}}run(e=()=>{}){new $({cb:e,shoot:()=>{this.render?.({camera:this.camera})},fps:this.fps,showFps:this.showFps}).start()}async buildAndRun(e=()=>{}){await this.build(),this.run(e)}}export{V as R};
