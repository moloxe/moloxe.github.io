...
`;function I({sdWGSL:a,materialSdFunctions:e}){function t(s,i){const c=`vec3<f32>(U.material${i}Px, U.material${i}Py, U.material${i}Pz)`,l=`vec3<f32>(U.material${i}Rx, U.material${i}Ry, U.material${i}Rz)`;return`
...
`)}
      ...
      return material;
    `;return a=a.replace("// #SD-MATERIALS-FUNC",o),a}function R({materialSdFunctions:a,sdWGSL:e}){function t(r){return`dist = min(dist, sdMaterial${r}(pos));`}const n=`
      var dist: f32 = INF;
      var accDist: f32;
...
`)}
      ...
      return dist;`;return e=e.replace("// #MAP",n),e}function z({materialLightFunctions:a,sdWGSL:e}){function t(s,i){return s||(s=`
...
`);e=e.replace("// #LIGHT-INDIVIDUAL-MATERIALS",n);function r(s){return`
      ...
      ${a.map((s,i)=>r(i)).join(`
`)}
      return finalColor;`;return e=e.replace("// #LIGHT-MATERIALS-FUNC",o),e}function B(a){let e=_;const t=a.map(r=>r.sdFunc),n=a.map(r=>r.lightFunc);return e=I({sdWGSL:e,materialSdFunctions:t}),e=R({materialSdFunctions:t,sdWGSL:e}),e=z({sdWGSL:e,materialLightFunctions:n}),e}function T({functions:a,rtUniformKeys:e,materialFuncs:t,nTextures:n,usePrevFrameTex:r}){let o=U;if(o=o.replace("UNIFORMS: f32, // #UNIFORMS",e.map(s=>`${s}: f32,`).join(`
`)),n>0){let s=function(c){return`
        ...
        }`};const i=new Array(n).fill(0).map((c,l)=>s(l)).join(`
`);o=o.replace("// #GROUP-1-BINDING-X",i)}if(r&&(o=o.replace("// #GROUP-2-BINDING-X",`
      ...
      }`)),a&&(o=o.replace("// #FUNCTIONS",a)),t.length>0){const s=B(t);o=o.replace("// #RAY_MARCH_FUNCTIONS",s)}return o}function M({main:a,functions:e,rtUniformKeys:t,materialFuncs:n,nTextures:r,usePrevFrameTex:o,useInterlacing:s}){const i=T({functions:e,materialFuncs:n,nTextures:r,rtUniformKeys:t,usePrevFrameTex:o});n.length>0&&!a&&(a=`
  ...
  `;return s&&(c=c.replace("// #INTERLACING",`
    ...
    `)),c}class F{device;buffer;uniform={};uniformBindGroupLayout;uniformBindGroup;constructor(e,t){this.device=e;const n=Object.keys(t);this.buffer=e.createBuffer({size:n.length*4,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),n.forEach((r,o)=>{this.uniform[r]={value:t[r],offSet:o*4},this.set(r,t[r])}),this.uniformBindGroupLayout=e.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.FRAGMENT,sampler:{type:"non-filtering"}}]}),this.uniformBindGroup=this.device.createBindGroup({layout:this.uniformBindGroupLayout,entries:[{binding:0,resource:{buffer:this.getBuffer()}},{binding:1,resource:this.device.createSampler({addressModeU:"repeat",addressModeV:"repeat",magFilter:"nearest",minFilter:"nearest"})}]})}getBuffer(){return this.buffer}getKeysSortedByOffset(){return Object.keys(this.uniform).toSorted((e,t)=>this.uniform[e].offSet-this.uniform[t].offSet)}set(e,t){if(!this.uniform[e])throw new Error(`Uniform ${e} not found`);this.uniform[e].value=t,this.device.queue.writeBuffer(this.buffer,this.uniform[e].offSet,new Float32Array([t]))}get(e){if(!this.uniform[e])throw new Error(`Uniform ${e} not found`);return this.uniform[e].value}getCustomBindingsForCollisions(e){const t=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.COMPUTE,sampler:{type:"non-filtering"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}}]}),n=this.device.createBindGroup({layout:t,entries:[{binding:0,resource:{buffer:this.getBuffer()}},{binding:1,resource:this.device.createSampler({addressModeU:"repeat",addressModeV:"repeat",magFilter:"nearest",minFilter:"nearest"})},{binding:2,resource:{buffer:e}}]});return{uniformBindGroupLayout:t,uniformBindGroup:n}}}class E{texs;device;presentationFormat;textures;textureEntries;textureBindGroupLayout;constructor(e,t,n){this.texs=n,this.device=e,this.presentationFormat=t,this.textures=this.texs.map(({width:r,height:o})=>this.device.createTexture({size:[r,o,1],format:this.presentationFormat,usage:GPUTextureUsage.COPY_DST|GPUTextureUsage.TEXTURE_BINDING})),this.textureEntries=this.textures.map((r,o)=>({binding:o,resource:r.createView()})),this.textureBindGroupLayout=e.createBindGroupLayout({entries:n.map((r,o)=>({binding:o,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"unfilterable-float"}}))})}setTexture(e,t){const n=this.texs[e].width,r=this.texs[e].height,o=this.textures[e];this.device.queue.writeTexture({texture:o},t,{offset:0,bytesPerRow:n*4,rowsPerImage:r},[n,r,1])}getTextureEntries(){return this.textureEntries}length(){return this.texs.length}}class A{pingPongBindGroupLayout;bindGroupPingIn;bindGroupPongIn;currentReadTexture;currentReadTextureView;currentWriteTexture;currentWriteTextureView;currentBindGroupIn;offscreenTextureSize;constructor(e,t,n){this.offscreenTextureSize=[e.width,e.height,1];const r=t.createTexture({size:this.offscreenTextureSize,format:n,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),o=t.createTexture({size:this.offscreenTextureSize,format:n,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),s=r.createView(),i=o.createView();this.pingPongBindGroupLayout=t.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"unfilterable-float"}}]}),this.bindGroupPingIn=t.createBindGroup({layout:this.pingPongBindGroupLayout,entries:[{binding:0,resource:r.createView()}]}),this.bindGroupPongIn=t.createBindGroup({layout:this.pingPongBindGroupLayout,entries:[{binding:0,resource:o.createView()}]}),this.currentReadTexture=r,this.currentReadTextureView=s,this.currentWriteTexture=o,this.currentWriteTextureView=i,this.currentBindGroupIn=this.bindGroupPingIn}preSubmit(){}swap(){const e=this.currentReadTexture,t=this.currentReadTextureView,n=this.currentBindGroupIn;this.currentReadTexture=this.currentWriteTexture,this.currentReadTextureView=this.currentWriteTextureView,this.currentBindGroupIn=n===this.bindGroupPingIn?this.bindGroupPongIn:this.bindGroupPingIn,this.currentWriteTexture=e,this.currentWriteTextureView=t}}async function D({device:a,presentationFormat:e,context:t,canvas:n,main:r,materialFuncs:o,functions:s,rtUniform:i,rtTexture:c,usePrevFrameTex:l,useInterlacing:p,bindGroupLayouts:h}){let f=null;l&&(f=new A(n,a,e),h.push(f.pingPongBindGroupLayout));const u=M({main:r,functions:s,materialFuncs:o,rtUniformKeys:i.getKeysSortedByOffset(),nTextures:c.length(),usePrevFrameTex:l,useInterlacing:p}),v=a.createRenderPipeline({layout:a.createPipelineLayout({bindGroupLayouts:h}),vertex:{module:a.createShaderModule({code:w}),entryPoint:"main"},fragment:{module:a.createShaderModule({code:u}),entryPoint:"main",targets:[{format:e}]},primitive:{topology:"triangle-strip",frontFace:"ccw",stripIndexFormat:"uint32"}});let y=0;const P=performance.now()/1e3;function b(m){const x=performance.now()/1e3-P;i.set("frame",y),i.set("time",x),i.set("aspectRatio",n.width/n.height),i.set("width",n.width),i.set("height",n.height),i.set("camPosX",m.pos.x),i.set("camPosY",m.pos.y),i.set("camPosZ",m.pos.z),i.set("camSphericalR",m.spherical.radius),i.set("camSphericalT",m.spherical.theta),i.set("camSphericalP",m.spherical.phi),i.set("camFov",m.fov)}const S=a.createBindGroup({layout:v.getBindGroupLayout(1),entries:c.getTextureEntries()});function G({camera:m}){b(m);const x={colorAttachments:[{view:f?f.currentWriteTextureView:t.getCurrentTexture().createView(),loadOp:"clear",clearValue:{r:0,g:0,b:0,a:1},storeOp:"store"}]},g=a.createCommandEncoder(),d=g.beginRenderPass(x);d.setPipeline(v),d.setBindGroup(0,i.uniformBindGroup),d.setBindGroup(1,S),f&&d.setBindGroup(2,f.currentBindGroupIn),d.draw(4,1,0,0),d.end(),f&&g.copyTextureToTexture({texture:f.currentWriteTexture},{texture:t.getCurrentTexture()},f.offscreenTextureSize),a.queue.submit([g.finish()]),f&&f.swap(),y++}return G}async function N(){const a=await navigator.gpu.requestAdapter({powerPreference:"high-performance"});if(!a)throw new Error("No adapter found");const e=a.features.has("texture-compression-bc");return e||console.warn("shader-f16 not available"),{device:await a.requestDevice({requiredFeatures:e?["shader-f16"]:[]}),presentationFormat:navigator.gpu.getPreferredCanvasFormat()}}class L{setPos;setRot;setColor;setCollisionGroup;checkForCollisions;collisionGroup=-1;index;rt;constructor({material:e,rt:t,index:n}){this.rt=t,e.pos||(e.pos={x:0,y:0,z:0}),e.color||(e.color={r:1,g:1,b:1}),e.rotation||(e.rotation={x:0,y:0,z:0});const r=t.registerUniform(`material${n}Px`,e.pos.x),o=t.registerUniform(`material${n}Py`,e.pos.y),s=t.registerUniform(`material${n}Pz`,e.pos.z);this.setPos=u=>{u.x!==void 0&&r(u.x),u.y!==void 0&&o(u.y),u.z!==void 0&&s(u.z)};const i=t.registerUniform(`material${n}Rx`,e.rotation.x),c=t.registerUniform(`material${n}Ry`,e.rotation.y),l=t.registerUniform(`material${n}Rz`,e.rotation.z);this.setRot=u=>{u.x!==void 0&&i(u.x),u.y!==void 0&&c(u.y),u.z!==void 0&&l(u.z)};const p=t.registerUniform(`material${n}Cr`,e.color.r),h=t.registerUniform(`material${n}Cg`,e.color.g),f=t.registerUniform(`material${n}Cb`,e.color.b);this.setColor=u=>{u.r!==void 0&&p(u.r),u.g!==void 0&&h(u.g),u.b!==void 0&&f(u.b)},e.enableCollisions&&(this.collisionGroup=n),this.setCollisionGroup=t.registerUniform(`material${n}CollisionGroup`,this.collisionGroup),this.index=n}checkCollision(){return this.rt.checkCollision(this.index)}getIndex(){return this.index}}function $({functions:a,rtUniformKeys:e,materialFuncs:t,nTextures:n,usePrevFrameTex:r}){const o=T({functions:a,materialFuncs:t,nTextures:n,rtUniformKeys:e,usePrevFrameTex:r}),s=t.map(l=>l.sdFunc);function i(l,p){return`
...
`)}
  ...
  `}class O{device;computePipeline;paramsBindGroup;paramsBuffer;uniformBindGroup;resultBufferSize;resultBuffer;rtTexture;constructor({device:e,functions:t,rtUniformKeys:n,materialFuncs:r,nTextures:o,usePrevFrameTex:s,rtUniform:i,rtTexture:c}){this.device=e,this.rtTexture=c;const l=5;this.resultBufferSize=Float32Array.BYTES_PER_ELEMENT*l,this.resultBuffer=e.createBuffer({size:this.resultBufferSize,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC});const{uniformBindGroupLayout:p,uniformBindGroup:h}=i.getCustomBindingsForCollisions(this.resultBuffer);this.uniformBindGroup=h,this.paramsBuffer=e.createBuffer({size:32,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});const f=e.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}}]});this.paramsBindGroup=e.createBindGroup({layout:f,entries:[{binding:0,resource:{buffer:this.paramsBuffer}}]});const u=[p,this.rtTexture.textureBindGroupLayout,f],v=$({functions:t,rtUniformKeys:n,materialFuncs:r,nTextures:o,usePrevFrameTex:s});this.computePipeline=e.createComputePipeline({layout:e.createPipelineLayout({bindGroupLayouts:u}),compute:{module:e.createShaderModule({code:v}),entryPoint:"main"}})}async checkCollision(e,t){this.device.queue.writeBuffer(this.paramsBuffer,0,new Int32Array([e])),this.device.queue.writeBuffer(this.paramsBuffer,16,new Float32Array([t.x,t.y,t.z]));const n=this.device.createCommandEncoder(),r=n.beginComputePass();r.setPipeline(this.computePipeline),r.setBindGroup(0,this.uniformBindGroup);const o=this.device.createBindGroup({layout:this.computePipeline.getBindGroupLayout(1),entries:this.rtTexture.getTextureEntries()});r.setBindGroup(1,o),r.setBindGroup(2,this.paramsBindGroup),r.dispatchWorkgroups(1),r.end();const s=this.device.createBuffer({size:this.resultBufferSize,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});n.copyBufferToBuffer(this.resultBuffer,0,s,0,this.resultBufferSize),this.device.queue.submit([n.finish()]),await s.mapAsync(GPUMapMode.READ);const i=new Float32Array(s.getMappedRange());try{const c=i[0],l=i[1],p={x:i[2],y:i[3],z:i[4]};return{materialIndex:c,collisionDist:l,collisionNormal:p}}finally{s.unmap()}}}class X{shoot;fps;fpsCounter=0;showFps;cb;constructor({cb:e,shoot:t,fps:n,showFps:r}){this.shoot=t,this.cb=e,this.fps=n,this.showFps=r}startFpsCounter(){const e=document.createElement("div");e.setAttribute("style",` position: absolute;
        ...
        pointer-events: none;`),document.body.appendChild(e),setInterval(()=>{e.innerText=`FPS: ${this.fpsCounter}`,this.fpsCounter=0},1e3)}start(){this.showFps&&this.startFpsCounter();const e=this.cb,t=this.shoot,n=()=>{this.fpsCounter++},r=this.fps;let o=async()=>{requestAnimationFrame(o),await e(),t(),n()};if(r!==void 0){const s=1e3/r;let i=0;o=async c=>{requestAnimationFrame(o);const l=c-i;l>s&&(i=c-l%s,await e(),t(),n())}}requestAnimationFrame(o)}}function k(a){const e=document.createElement("canvas");return e.height=a??window.innerHeight,e.width=e.height*(window.innerWidth/window.innerHeight),e.style.width="100vw",e.style.height="100vh",a&&(e.style.imageRendering="pixelated"),document.body.appendChild(e),e}class Y{canvas;main;functions;render;initalCustomUniforms={};setUniform;materialFuncs=[];usePrevFrameTex;useInterlacing;setDeviceTexure;texs;fps;showFps;rtCollision;rtUniform;camera={pos:{x:0,y:0,z:0},spherical:{radius:0,theta:0,phi:0},fov:90};constructor({main:e,functions:t,texs:n,usePrevFrameTex:r,useInterlacing:o,fps:s,height:i,showFps:c}={}){this.canvas=k(i),this.main=e,this.functions=t,this.texs=n??[],this.useInterlacing=o,this.usePrevFrameTex=this.useInterlacing||r,this.fps=s,this.showFps=c}registerMaterial(e){if(this.render)throw new Error("Render already built");const t=this.materialFuncs.length,n=new L({rt:this,material:e,index:t});return this.materialFuncs.push({sdFunc:e.sdFunc,lightFunc:e.lightFunc}),n}setTex(e,t){if(!this.setDeviceTexure)throw new Error("Render not built");this.setDeviceTexure(e,t)}registerUniform(e,t){if(this.render)throw new Error("Render already built");return this.initalCustomUniforms[e]=t??0,r=>{if(!this.setUniform)throw new Error("Render not built");this.setUniform(e,r),this.initalCustomUniforms[e]=r}}async build(){const{device:e,presentationFormat:t}=await N(),n=this.canvas.getContext("webgpu");if(!n)throw new Error("No context found");n.configure({device:e,format:t,alphaMode:"opaque",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.COPY_DST});const r=new F(e,{frame:0,time:0,aspectRatio:this.canvas.width/this.canvas.height,width:this.canvas.width,height:this.canvas.height,camPosX:this.camera.pos.x??0,camPosY:this.camera.pos.y??0,camPosZ:this.camera.pos.z??0,camSphericalR:this.camera.spherical.radius??0,camSphericalT:this.camera.spherical.theta??0,camSphericalP:this.camera.spherical.phi??0,camFov:this.camera.fov??90,...this.initalCustomUniforms});this.rtUniform=r;const o=new E(e,t,this.texs),s=[r.uniformBindGroupLayout,o.textureBindGroupLayout];this.render=await D({device:e,bindGroupLayouts:s,presentationFormat:t,context:n,canvas:this.canvas,main:this.main,materialFuncs:this.materialFuncs,functions:this.functions,rtUniform:r,rtTexture:o,usePrevFrameTex:this.usePrevFrameTex,useInterlacing:this.useInterlacing}),this.setUniform=(i,c)=>{r.set(i,c)},this.setDeviceTexure=(i,c)=>{o.setTexture(i,c)},this.rtCollision=new O({device:e,rtUniformKeys:r.getKeysSortedByOffset(),functions:this.functions,materialFuncs:this.materialFuncs,nTextures:this.texs.length,usePrevFrameTex:this.usePrevFrameTex,rtUniform:r,rtTexture:o})}checkCollision(e){if(!this.rtCollision||!this.rtUniform)throw new Error("Render not built");const t=this.rtUniform.get(`material${e}Px`),n=this.rtUniform.get(`material${e}Py`),r=this.rtUniform.get(`material${e}Pz`);return this.rtCollision.checkCollision(e,{x:t,y:n,z:r})}async start(e){if(await this.build(),e?.onBuild?.(),e?.once){this.render?.({camera:this.camera});return}new X({cb:e?.onFrame??(()=>{}),shoot:()=>{this.render?.({camera:this.camera})},fps:this.fps,showFps:this.showFps}).start()}}export{Y as R};
