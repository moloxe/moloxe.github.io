...
`;function R(n){let e=I;const r=n.map(i=>i.sdFunc),o=n.map(i=>i.lightFunc);{const i=r.map((a,t)=>{const f=`vec3<f32>(U.material${t}Px, U.material${t}Py, U.material${t}Pz)`,u=`vec3<f32>(U.material${t}Rx, U.material${t}Ry, U.material${t}Rz)`;return`fn sdMaterial${t}(posIn: vec3<f32>) -> f32 {
        let mRotation = ${u};
        var pos = posIn - ${f};
...
`)}
      return finalColor;
    `;e=e.replace("return vec4<f32>(0.0); // #LIGHT-MATERIALS-FUNC",s)}return e}function G({main:n,functions:e,rtUniformKeys:r,materialFuncs:o,nTextures:i,usePrevFrameTex:s,useInterlacing:a}){let t=z;if(t=t.replace("UNIFORMS: f32, // #UNIFORMS",r.map(f=>`${f}: f32,`).join(`
`)),i>0){const f=new Array(i).fill(0).map((u,h)=>`
...
`);t=t.replace("// #GROUP-1-BINDING-X",f)}if(s&&(t=t.replace("// #GROUP-2-BINDING-X",`@group(2) @binding(0) var prevFrameTex: texture_2d<f32>;
      ...
      }`)),t=t.replace("// #COMMON",P),e&&(t=t.replace("// #FUNCTIONS",e)),o.length>0){const f=R(o);t=t.replace("// #RAY_MARCH_FUNCTIONS",f),n||(n=`
      ...
      `)}return a&&(t=t.replace("// #INTERLACING",` let _prev_pix_ = getPrevFrameTexSample(uv);
        ...
        return _prev_pix_;
        }`)),n&&(t=t.replace("return vec4<f32>(0.0, 0.0, 0.0, 1.0); // #MAIN",n)),t}class C{device;buffer;uniform={};uniformBindGroupLayout;uniformBindGroup;constructor(e,r){this.device=e;const o=Object.keys(r);this.buffer=e.createBuffer({size:o.length*4,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),o.forEach((i,s)=>{this.uniform[i]={value:r[i],offSet:s*4},this.set(i,r[i])}),this.uniformBindGroupLayout=e.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.FRAGMENT,sampler:{type:"non-filtering"}}]}),this.uniformBindGroup=this.device.createBindGroup({layout:this.uniformBindGroupLayout,entries:[{binding:0,resource:{buffer:this.getBuffer()}},{binding:1,resource:this.device.createSampler({addressModeU:"repeat",addressModeV:"repeat",magFilter:"nearest",minFilter:"nearest"})}]})}getBuffer(){return this.buffer}getKeysSortedByOffset(){return Object.keys(this.uniform).toSorted((e,r)=>this.uniform[e].offSet-this.uniform[r].offSet)}set(e,r){if(!this.uniform[e])throw new Error(`Uniform ${e} not found`);this.uniform[e].value=r,this.device.queue.writeBuffer(this.buffer,this.uniform[e].offSet,new Float32Array([r]))}get(e){if(!this.uniform[e])throw new Error(`Uniform ${e} not found`);return this.uniform[e].value}}class U{texs;device;presentationFormat;textures;textureEntries;textureBindGroupLayout;constructor(e,r,o){this.texs=o,this.device=e,this.presentationFormat=r,this.textures=this.texs.map(({width:i,height:s})=>this.device.createTexture({size:[i,s,1],format:this.presentationFormat,usage:GPUTextureUsage.COPY_DST|GPUTextureUsage.TEXTURE_BINDING})),this.textureEntries=this.textures.map((i,s)=>({binding:s,resource:i.createView()})),this.textureBindGroupLayout=e.createBindGroupLayout({entries:o.map((i,s)=>({binding:s,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"unfilterable-float"}}))})}setTexture(e,r){const o=this.texs[e].width,i=this.texs[e].height,s=this.textures[e];this.device.queue.writeTexture({texture:s},r,{offset:0,bytesPerRow:o*4,rowsPerImage:i},[o,i,1]),this.textureEntries[e].resource=s.createView()}getTextureEntries(){return this.textureEntries}length(){return this.texs.length}}class _{pingPongBindGroupLayout;bindGroupPingIn;bindGroupPongIn;currentReadTexture;currentReadTextureView;currentWriteTexture;currentWriteTextureView;currentBindGroupIn;offscreenTextureSize;constructor(e,r,o){this.offscreenTextureSize=[e.width,e.height,1];const i=r.createTexture({size:this.offscreenTextureSize,format:o,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),s=r.createTexture({size:this.offscreenTextureSize,format:o,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),a=i.createView(),t=s.createView();this.pingPongBindGroupLayout=r.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"unfilterable-float"}}]}),this.bindGroupPingIn=r.createBindGroup({layout:this.pingPongBindGroupLayout,entries:[{binding:0,resource:i.createView()}]}),this.bindGroupPongIn=r.createBindGroup({layout:this.pingPongBindGroupLayout,entries:[{binding:0,resource:s.createView()}]}),this.currentReadTexture=i,this.currentReadTextureView=a,this.currentWriteTexture=s,this.currentWriteTextureView=t,this.currentBindGroupIn=this.bindGroupPingIn}preSubmit(){}swap(){const e=this.currentReadTexture,r=this.currentReadTextureView,o=this.currentBindGroupIn;this.currentReadTexture=this.currentWriteTexture,this.currentReadTextureView=this.currentWriteTextureView,this.currentBindGroupIn=o===this.bindGroupPingIn?this.bindGroupPongIn:this.bindGroupPingIn,this.currentWriteTexture=e,this.currentWriteTextureView=r}}async function F({device:n,presentationFormat:e,context:r,canvas:o,main:i,materialFuncs:s,functions:a,rtUniform:t,rtTexture:f,usePrevFrameTex:u,useInterlacing:h}){const m=[t.uniformBindGroupLayout,f.textureBindGroupLayout];let l=null;u&&(l=new _(o,n,e),m.push(l.pingPongBindGroupLayout));const x=G({main:i,functions:a,materialFuncs:s,rtUniformKeys:t.getKeysSortedByOffset(),nTextures:f.length(),usePrevFrameTex:u,useInterlacing:h}),d=n.createRenderPipeline({layout:n.createPipelineLayout({bindGroupLayouts:m}),vertex:{module:n.createShaderModule({code:S}),entryPoint:"main"},fragment:{module:n.createShaderModule({code:x}),entryPoint:"main",targets:[{format:e}]},primitive:{topology:"triangle-strip",frontFace:"ccw",stripIndexFormat:"uint32"}});let c=0;function y(p){t.set("frame",c),t.set("time",performance.now()/1e3),t.set("aspectRatio",o.width/o.height),t.set("width",o.width),t.set("height",o.height),t.set("camPosX",p.pos.x),t.set("camPosY",p.pos.y),t.set("camPosZ",p.pos.z),t.set("camSphericalR",p.spherical.radius),t.set("camSphericalT",p.spherical.theta),t.set("camSphericalP",p.spherical.phi),t.set("camFov",p.fov)}function T({camera:p}){y(p);const w={colorAttachments:[{view:l?l.currentWriteTextureView:r.getCurrentTexture().createView(),loadOp:"clear",clearValue:{r:0,g:0,b:0,a:1},storeOp:"store"}]},g=n.createCommandEncoder(),v=g.beginRenderPass(w);v.setPipeline(d),v.setBindGroup(0,t.uniformBindGroup);const b=n.createBindGroup({layout:d.getBindGroupLayout(1),entries:f.getTextureEntries()});v.setBindGroup(1,b),l&&v.setBindGroup(2,l.currentBindGroupIn),v.draw(4,1,0,0),v.end(),l&&g.copyTextureToTexture({texture:l.currentWriteTexture},{texture:r.getCurrentTexture()},l.offscreenTextureSize),n.queue.submit([g.finish()]),l&&l.swap(),c++}return T}async function M(){const n=await navigator.gpu.requestAdapter();if(!n)throw new Error("No adapter found");const e=n.features.has("texture-compression-bc");return e||console.warn("shader-f16 not available"),{device:await n.requestDevice({requiredFeatures:e?["shader-f16"]:[]}),presentationFormat:navigator.gpu.getPreferredCanvasFormat()}}function N(n,e,r){n.pos||(n.pos={x:0,y:0,z:0}),n.color||(n.color={r:1,g:1,b:1}),n.rotation||(n.rotation={x:0,y:0,z:0});const o=e.registerUniform(`material${r}Px`,n.pos.x),i=e.registerUniform(`material${r}Py`,n.pos.y),s=e.registerUniform(`material${r}Pz`,n.pos.z),a=c=>{c.x!==void 0&&o(c.x),c.y!==void 0&&i(c.y),c.z!==void 0&&s(c.z)},t=e.registerUniform(`material${r}Rx`,n.rotation.x),f=e.registerUniform(`material${r}Ry`,n.rotation.y),u=e.registerUniform(`material${r}Rz`,n.rotation.z),h=c=>{c.x!==void 0&&t(c.x),c.y!==void 0&&f(c.y),c.z!==void 0&&u(c.z)},m=e.registerUniform(`material${r}Cr`,n.color.r),l=e.registerUniform(`material${r}Cg`,n.color.g),x=e.registerUniform(`material${r}Cb`,n.color.b);return{setPos:a,setRot:h,setColor:c=>{c.r!==void 0&&m(c.r),c.g!==void 0&&l(c.g),c.b!==void 0&&x(c.b)}}}class D{shoot;fps;fpsCounter=0;showFps;cb;constructor({cb:e,shoot:r,fps:o,showFps:i}){this.shoot=r,this.cb=e,this.fps=o,this.showFps=i}startFpsCounter(){const e=document.createElement("div");e.setAttribute("style",` position: absolute;
        top: 0;
        left: 0;
        color: green;
        background: black;
        ...
        pointer-events: none;`),document.body.appendChild(e),setInterval(()=>{e.innerText=`FPS: ${this.fpsCounter}`,this.fpsCounter=0},1e3)}start(){this.showFps&&this.startFpsCounter();const e=this.cb,r=this.shoot,o=()=>{this.fpsCounter++},i=this.fps;let s=()=>{requestAnimationFrame(s),e(),r(),o()};if(i!==void 0){const a=1e3/i;let t=0;s=f=>{requestAnimationFrame(s);const u=f-t;u>a&&(t=f-u%a,e(),r(),o())}}requestAnimationFrame(s)}}function L(n){const e=document.createElement("canvas");return e.height=n??window.innerHeight,e.width=e.height*(window.innerWidth/window.innerHeight),e.style.width="100vw",e.style.height="100vh",n&&(e.style.imageRendering="pixelated"),document.body.appendChild(e),e}class E{canvas;main;functions;render;initalCustomUniforms={};setUniform;materialFuncs=[];usePrevFrameTex;useInterlacing;setDeviceTexure;texs;fps;showFps;camera={pos:{x:0,y:0,z:0},spherical:{radius:0,theta:0,phi:0},fov:90};constructor({main:e,functions:r,texs:o,usePrevFrameTex:i,useInterlacing:s,fps:a,height:t,showFps:f}={}){this.canvas=L(t),this.main=e,this.functions=r,this.texs=o??[],this.useInterlacing=s,this.usePrevFrameTex=this.useInterlacing||i,this.fps=a,this.showFps=f}registerMaterial(e){if(this.render)throw new Error("Render already built");const r=this.materialFuncs.length,o=N(e,this,r);return this.materialFuncs.push({sdFunc:e.sdFunc,lightFunc:e.lightFunc}),o}setTex(e,r){if(!this.setDeviceTexure)throw new Error("Render not built");this.setDeviceTexure(e,r)}registerUniform(e,r){if(this.render)throw new Error("Render already built");return this.initalCustomUniforms[e]=r??0,i=>{if(!this.setUniform)throw new Error("Render not built");this.setUniform(e,i),this.initalCustomUniforms[e]=i}}async build(){const{device:e,presentationFormat:r}=await M(),o=this.canvas.getContext("webgpu");if(!o)throw new Error("No context found");o.configure({device:e,format:r,alphaMode:"opaque",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.COPY_DST});const i=new C(e,{frame:0,time:0,aspectRatio:this.canvas.width/this.canvas.height,width:this.canvas.width,height:this.canvas.height,camPosX:this.camera.pos.x??0,camPosY:this.camera.pos.y??0,camPosZ:this.camera.pos.z??0,camSphericalR:this.camera.spherical.radius??0,camSphericalT:this.camera.spherical.theta??0,camSphericalP:this.camera.spherical.phi??0,camFov:this.camera.fov??90,...this.initalCustomUniforms}),s=new U(e,r,this.texs);this.render=await F({device:e,presentationFormat:r,context:o,canvas:this.canvas,main:this.main,materialFuncs:this.materialFuncs,functions:this.functions,rtUniform:i,rtTexture:s,usePrevFrameTex:this.usePrevFrameTex,useInterlacing:this.useInterlacing}),this.setUniform=(a,t)=>{i.set(a,t)},this.setDeviceTexure=(a,t)=>{s.setTexture(a,t)}}run(e=()=>{}){new D({cb:e,shoot:()=>{this.render?.({camera:this.camera})},fps:this.fps,showFps:this.showFps}).start()}async buildAndRun(e=()=>{}){await this.build(),this.run(e)}}export{E as R};
