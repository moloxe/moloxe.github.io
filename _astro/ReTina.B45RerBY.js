...
`;function _(n){let e=U;const r=n.map(i=>i.sdFunc),s=n.map(i=>i.lightFunc);{const i=r.map((a,t)=>{const f=`vec3<f32>(U.material${t}Px, U.material${t}Py, U.material${t}Pz)`,p=`vec3<f32>(U.material${t}Rx, U.material${t}Ry, U.material${t}Rz)`;return`fn sdMaterial${t}(posIn: vec3<f32>) -> f32 {
        let mRotation = ${p};
        var pos = posIn - ${f};
...
`);e=e.replace("// #SD-INDIVIDUAL-MATERIALS",i);const o=`
...
`)}
    return material;
  `;e=e.replace("return SdMaterial(-1, 0., vec3<f32>(0.), vec3<f32>(1.)); // #SD-MATERIALS-FUNC",o)}{const i=`
    var dist: f32 = 1e10;
    var accDist: f32;
    ${r.map((o,a)=>`dist = min(dist, sdMaterial${a}(pos));`).join(`
`)}
    return dist;
  `;e=e.replace("return 0.; // #MAP",i)}{const i=s.map((a,t)=>`
...
`);e=e.replace("// #LIGHT-INDIVIDUAL-MATERIALS",i);const o=`
...
`)}
      return finalColor;
    `;e=e.replace("return vec4<f32>(0.0); // #LIGHT-MATERIALS-FUNC",o)}return e}function F({main:n,functions:e,rtUniformKeys:r,materialFuncs:s,nTextures:i,usePrevFrameTex:o,useInterlacing:a}){let t=C;if(t=t.replace("UNIFORMS: f32, // #UNIFORMS",r.map(f=>`${f}: f32,`).join(`
`)),i>0){const f=new Array(i).fill(0).map((p,h)=>`
        ...
        `).join(`
`);t=t.replace("// #GROUP-1-BINDING-X",f)}if(o&&(t=t.replace("// #GROUP-2-BINDING-X",`@group(2) @binding(0) var prevFrameTex: texture_2d<f32>;
      fn getPrevFrameTexSample(uv: vec2<f32>) -> vec4<f32> {
        return textureSample(prevFrameTex, u_sampler, uv);
      }`)),t=t.replace("// #COMMON",G),e&&(t=t.replace("// #FUNCTIONS",e)),s.length>0){const f=_(s);t=t.replace("// #RAY_MARCH_FUNCTIONS",f),n||(n=`
      ...
      `)}return a&&(t=t.replace("// #INTERLACING",` let _prev_pix_ = getPrevFrameTexSample(uv);
        ...
        return _prev_pix_;
        }`)),n&&(t=t.replace("return vec4<f32>(0.0, 0.0, 0.0, 1.0); // #MAIN",n)),t}class M{device;buffer;uniform={};uniformBindGroupLayout;uniformBindGroup;constructor(e,r){this.device=e;const s=Object.keys(r);this.buffer=e.createBuffer({size:s.length*4,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),s.forEach((i,o)=>{this.uniform[i]={value:r[i],offSet:o*4},this.set(i,r[i])}),this.uniformBindGroupLayout=e.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.FRAGMENT,sampler:{type:"non-filtering"}}]}),this.uniformBindGroup=this.device.createBindGroup({layout:this.uniformBindGroupLayout,entries:[{binding:0,resource:{buffer:this.getBuffer()}},{binding:1,resource:this.device.createSampler({addressModeU:"repeat",addressModeV:"repeat",magFilter:"nearest",minFilter:"nearest"})}]})}getBuffer(){return this.buffer}getKeysSortedByOffset(){return Object.keys(this.uniform).toSorted((e,r)=>this.uniform[e].offSet-this.uniform[r].offSet)}set(e,r){if(!this.uniform[e])throw new Error(`Uniform ${e} not found`);this.uniform[e].value=r,this.device.queue.writeBuffer(this.buffer,this.uniform[e].offSet,new Float32Array([r]))}get(e){if(!this.uniform[e])throw new Error(`Uniform ${e} not found`);return this.uniform[e].value}}class N{texs;device;presentationFormat;textures;textureEntries;textureBindGroupLayout;constructor(e,r,s){this.texs=s,this.device=e,this.presentationFormat=r,this.textures=this.texs.map(({width:i,height:o})=>this.device.createTexture({size:[i,o,1],format:this.presentationFormat,usage:GPUTextureUsage.COPY_DST|GPUTextureUsage.TEXTURE_BINDING})),this.textureEntries=this.textures.map((i,o)=>({binding:o,resource:i.createView()})),this.textureBindGroupLayout=e.createBindGroupLayout({entries:s.map((i,o)=>({binding:o,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"unfilterable-float"}}))})}setTexture(e,r){const s=this.texs[e].width,i=this.texs[e].height,o=this.textures[e];this.device.queue.writeTexture({texture:o},r,{offset:0,bytesPerRow:s*4,rowsPerImage:i},[s,i,1]),this.textureEntries[e].resource=o.createView()}getTextureEntries(){return this.textureEntries}}class D{pingPongBindGroupLayout;bindGroupPingIn;bindGroupPongIn;currentReadTexture;currentReadTextureView;currentWriteTexture;currentWriteTextureView;currentBindGroupIn;offscreenTextureSize;constructor(e,r,s){this.offscreenTextureSize=[e.width,e.height,1];const i=r.createTexture({size:this.offscreenTextureSize,format:s,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),o=r.createTexture({size:this.offscreenTextureSize,format:s,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),a=i.createView(),t=o.createView();this.pingPongBindGroupLayout=r.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"unfilterable-float"}}]}),this.bindGroupPingIn=r.createBindGroup({layout:this.pingPongBindGroupLayout,entries:[{binding:0,resource:i.createView()}]}),this.bindGroupPongIn=r.createBindGroup({layout:this.pingPongBindGroupLayout,entries:[{binding:0,resource:o.createView()}]}),this.currentReadTexture=i,this.currentReadTextureView=a,this.currentWriteTexture=o,this.currentWriteTextureView=t,this.currentBindGroupIn=this.bindGroupPingIn}preSubmit(){}swap(){const e=this.currentReadTexture,r=this.currentReadTextureView,s=this.currentBindGroupIn;this.currentReadTexture=this.currentWriteTexture,this.currentReadTextureView=this.currentWriteTextureView,this.currentBindGroupIn=s===this.bindGroupPingIn?this.bindGroupPongIn:this.bindGroupPingIn,this.currentWriteTexture=e,this.currentWriteTextureView=r}}async function L({device:n,presentationFormat:e,context:r,canvas:s,main:i,materialFuncs:o,functions:a,rtUniform:t,texs:f,usePrevFrameTex:p,useInterlacing:h}){const m=new N(n,e,f),x=[t.uniformBindGroupLayout,m.textureBindGroupLayout];let l=null;p&&(l=new D(s,n,e),x.push(l.pingPongBindGroupLayout));const y=F({main:i,functions:a,materialFuncs:o,rtUniformKeys:t.getKeysSortedByOffset(),nTextures:f.length,usePrevFrameTex:p,useInterlacing:h}),c=n.createRenderPipeline({layout:n.createPipelineLayout({bindGroupLayouts:x}),vertex:{module:n.createShaderModule({code:R}),entryPoint:"main"},fragment:{module:n.createShaderModule({code:y}),entryPoint:"main",targets:[{format:e}]},primitive:{topology:"triangle-strip",frontFace:"ccw",stripIndexFormat:"uint32"}});let T=0;function w(u){t.set("frame",T),t.set("time",performance.now()/1e3),t.set("aspectRatio",s.width/s.height),t.set("width",s.width),t.set("height",s.height),t.set("camPosX",u.pos.x),t.set("camPosY",u.pos.y),t.set("camPosZ",u.pos.z),t.set("camSphericalR",u.spherical.radius),t.set("camSphericalT",u.spherical.theta),t.set("camSphericalP",u.spherical.phi),t.set("camFov",u.fov)}function b({camera:u}){w(u);const z={colorAttachments:[{view:l?l.currentWriteTextureView:r.getCurrentTexture().createView(),loadOp:"clear",clearValue:{r:0,g:0,b:0,a:1},storeOp:"store"}]},g=n.createCommandEncoder(),v=g.beginRenderPass(z);v.setPipeline(c),v.setBindGroup(0,t.uniformBindGroup);const I=n.createBindGroup({layout:c.getBindGroupLayout(1),entries:m.getTextureEntries()});v.setBindGroup(1,I),l&&v.setBindGroup(2,l.currentBindGroupIn),v.draw(4,1,0,0),v.end(),l&&g.copyTextureToTexture({texture:l.currentWriteTexture},{texture:r.getCurrentTexture()},l.offscreenTextureSize),n.queue.submit([g.finish()]),l&&l.swap(),T++}function P(u,d){t.set(u,d)}function S(u,d){m.setTexture(u,d)}return{render:b,setUniform:P,setTexture:S}}async function A(n){const r=await navigator.gpu.requestAdapter();if(!r)throw new Error("No adapter found");const s=r.features.has("texture-compression-bc");s||console.warn("shader-f16 not available");const i=await r.requestDevice({requiredFeatures:s?["shader-f16"]:[]}),o=navigator.gpu.getPreferredCanvasFormat(),a=n.getContext("webgpu");if(!a)throw new Error("No context found");return a.configure({device:i,format:o,alphaMode:"opaque",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.COPY_DST}),{context:a,device:i,presentationFormat:o}}function E(n,e,r){n.pos||(n.pos={x:0,y:0,z:0}),n.color||(n.color={r:1,g:1,b:1}),n.rotation||(n.rotation={x:0,y:0,z:0});const s=e.registerUniform(`material${r}Px`,n.pos.x),i=e.registerUniform(`material${r}Py`,n.pos.y),o=e.registerUniform(`material${r}Pz`,n.pos.z),a=c=>{c.x!==void 0&&s(c.x),c.y!==void 0&&i(c.y),c.z!==void 0&&o(c.z)},t=e.registerUniform(`material${r}Rx`,n.rotation.x),f=e.registerUniform(`material${r}Ry`,n.rotation.y),p=e.registerUniform(`material${r}Rz`,n.rotation.z),h=c=>{c.x!==void 0&&t(c.x),c.y!==void 0&&f(c.y),c.z!==void 0&&p(c.z)},m=e.registerUniform(`material${r}Cr`,n.color.r),x=e.registerUniform(`material${r}Cg`,n.color.g),l=e.registerUniform(`material${r}Cb`,n.color.b);return{setPos:a,setRot:h,setColor:c=>{c.r!==void 0&&m(c.r),c.g!==void 0&&x(c.g),c.b!==void 0&&l(c.b)}}}class B{shoot;fps;fpsCounter=0;showFps;cb;constructor({cb:e,shoot:r,fps:s,showFps:i}){this.shoot=r,this.cb=e,this.fps=s,this.showFps=i}startFpsCounter(){const e=document.createElement("div");e.setAttribute("style",` position: absolute;
        top: 0;
        left: 0;
        color: green;
        background: black;
        ...
        pointer-events: none;`),document.body.appendChild(e),setInterval(()=>{e.innerText=`FPS: ${this.fpsCounter}`,this.fpsCounter=0},1e3)}start(){this.showFps&&this.startFpsCounter();const e=this.cb,r=this.shoot,s=()=>{this.fpsCounter++},i=this.fps;let o=()=>{requestAnimationFrame(o),e(),r(),s()};if(i!==void 0){const a=1e3/i;let t=0;o=f=>{requestAnimationFrame(o);const p=f-t;p>a&&(t=f-p%a,e(),r(),s())}}requestAnimationFrame(o)}}function X(n){const e=document.createElement("canvas");return e.height=n??window.innerHeight,e.width=e.height*(window.innerWidth/window.innerHeight),e.style.width="100vw",e.style.height="100vh",n&&(e.style.imageRendering="pixelated"),document.body.appendChild(e),e}class ${canvas;main;functions;render;initalCustomUniforms={};setUniform;materialFuncs=[];usePrevFrameTex;useInterlacing;setDeviceTexure;texs;fps;showFps;camera={pos:{x:0,y:0,z:0},spherical:{radius:0,theta:0,phi:0},fov:90};constructor({main:e,functions:r,texs:s,usePrevFrameTex:i,useInterlacing:o,fps:a,height:t,showFps:f}={}){this.canvas=X(t),this.main=e,this.functions=r,this.texs=s??[],this.useInterlacing=o,this.usePrevFrameTex=this.useInterlacing||i,this.fps=a,this.showFps=f}registerMaterial(e){if(this.render)throw new Error("Render already built");const r=this.materialFuncs.length,s=E(e,this,r);return this.materialFuncs.push({sdFunc:e.sdFunc,lightFunc:e.lightFunc}),s}setTex(e,r){if(!this.setDeviceTexure)throw new Error("Render not built");this.setDeviceTexure(e,r)}registerUniform(e,r){if(this.render)throw new Error("Render already built");return this.initalCustomUniforms[e]=r??0,i=>{if(!this.setUniform)throw new Error("Render not built");this.setUniform(e,i),this.initalCustomUniforms[e]=i}}async build(){const{device:e,context:r,presentationFormat:s}=await A(this.canvas),i=new M(e,{frame:0,time:0,aspectRatio:this.canvas.width/this.canvas.height,width:this.canvas.width,height:this.canvas.height,camPosX:this.camera.pos.x??0,camPosY:this.camera.pos.y??0,camPosZ:this.camera.pos.z??0,camSphericalR:this.camera.spherical.radius??0,camSphericalT:this.camera.spherical.theta??0,camSphericalP:this.camera.spherical.phi??0,camFov:this.camera.fov??90,...this.initalCustomUniforms}),{render:o,setUniform:a,setTexture:t}=await L({device:e,presentationFormat:s,context:r,canvas:this.canvas,main:this.main,materialFuncs:this.materialFuncs,functions:this.functions,rtUniform:i,texs:this.texs,usePrevFrameTex:this.usePrevFrameTex,useInterlacing:this.useInterlacing});this.setUniform=a,this.render=o,this.setDeviceTexure=t}run(e=()=>{}){new B({cb:e,shoot:()=>{this.render?.({camera:this.camera})},fps:this.fps,showFps:this.showFps}).start()}async buildAndRun(e=()=>{}){await this.build(),this.run(e)}}export{$ as R};
