...
`;function I({sdWGSL:a,materialSdFunctions:e}){function t(s,o){const c=`vec3<f32>(U.material${o}Px, U.material${o}Py, U.material${o}Pz)`,l=`vec3<f32>(U.material${o}Rx, U.material${o}Ry, U.material${o}Rz)`;return`
...
`)}
      ...
      return material;
    `;return a=a.replace("// #SD-MATERIALS-FUNC",i),a}function R({materialSdFunctions:a,sdWGSL:e}){function t(r){return`dist = min(dist, sdMaterial${r}(pos));`}const n=`
      var dist: f32 = INF;
      var accDist: f32;
...
`)}
      ...
      return dist;`;return e=e.replace("// #MAP",n),e}function z({materialLightFunctions:a,sdWGSL:e}){function t(s,o){return s||(s=`
...
`)),n>0){let s=function(c){return`
        ...
        }`};const o=new Array(n).fill(0).map((c,l)=>s(l)).join(`
`);i=i.replace("// #GROUP-1-BINDING-X",o)}if(r&&(i=i.replace("// #GROUP-2-BINDING-X",`
      ...
      }`)),a&&(i=i.replace("// #FUNCTIONS",a)),t.length>0){const s=B(t);i=i.replace("// #RAY_MARCH_FUNCTIONS",s)}return i}function M({main:a,functions:e,rtUniformKeys:t,materialFuncs:n,nTextures:r,usePrevFrameTex:i,useInterlacing:s}){const o=T({functions:e,materialFuncs:n,nTextures:r,rtUniformKeys:t,usePrevFrameTex:i});n.length>0&&!a&&(a=`
...
`)}
  ...
  `}class O{device;computePipeline;paramsBindGroup;paramsBuffer;uniformBindGroup;resultBufferSize;resultBuffer;rtTexture;constructor({device:e,functions:t,rtUniformKeys:n,materialFuncs:r,nTextures:i,usePrevFrameTex:s,rtUniform:o,rtTexture:c}){this.device=e,this.rtTexture=c;const l=5;this.resultBufferSize=Float32Array.BYTES_PER_ELEMENT*l,this.resultBuffer=e.createBuffer({size:this.resultBufferSize,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC});const{uniformBindGroupLayout:p,uniformBindGroup:h}=o.getCustomBindingsForCollisions(this.resultBuffer);this.uniformBindGroup=h,this.paramsBuffer=e.createBuffer({size:32,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});const f=e.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}}]});this.paramsBindGroup=e.createBindGroup({layout:f,entries:[{binding:0,resource:{buffer:this.paramsBuffer}}]});const u=[p,this.rtTexture.textureBindGroupLayout,f],v=$({functions:t,rtUniformKeys:n,materialFuncs:r,nTextures:i,usePrevFrameTex:s});this.computePipeline=e.createComputePipeline({layout:e.createPipelineLayout({bindGroupLayouts:u}),compute:{module:e.createShaderModule({code:v}),entryPoint:"main"}})}async checkCollision(e,t){this.device.queue.writeBuffer(this.paramsBuffer,0,new Int32Array([e])),this.device.queue.writeBuffer(this.paramsBuffer,16,new Float32Array([t.x,t.y,t.z]));const n=this.device.createCommandEncoder(),r=n.beginComputePass();r.setPipeline(this.computePipeline),r.setBindGroup(0,this.uniformBindGroup);const i=this.device.createBindGroup({layout:this.computePipeline.getBindGroupLayout(1),entries:this.rtTexture.getTextureEntries()});r.setBindGroup(1,i),r.setBindGroup(2,this.paramsBindGroup),r.dispatchWorkgroups(1),r.end();const s=this.device.createBuffer({size:this.resultBufferSize,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});n.copyBufferToBuffer(this.resultBuffer,0,s,0,this.resultBufferSize),this.device.queue.submit([n.finish()]),await s.mapAsync(GPUMapMode.READ);const o=new Float32Array(s.getMappedRange());try{const c=o[0],l=o[1],p={x:o[2],y:o[3],z:o[4]};return{materialIndex:c,collisionDist:l,collisionNormal:p}}finally{s.unmap()}}}class X{shoot;fps;fpsCounter=0;showFps;cb;constructor({cb:e,shoot:t,fps:n,showFps:r}){this.shoot=t,this.cb=e,this.fps=n,this.showFps=r}startFpsCounter(){const e=document.createElement("div");e.setAttribute("style",` position: absolute;
        top: 0;
        left: 0;
        color: green;
        background: black;
        ...
        pointer-events: none;`),document.body.appendChild(e),setInterval(()=>{e.innerText=`FPS: ${this.fpsCounter}`,this.fpsCounter=0},1e3)}start(){this.showFps&&this.startFpsCounter();const e=this.cb,t=this.shoot,n=()=>{this.fpsCounter++},r=this.fps;let i=async()=>{requestAnimationFrame(i),await e(),t(),n()};if(r!==void 0){const s=1e3/r;let o=0;i=async c=>{requestAnimationFrame(i);const l=c-o;l>s&&(o=c-l%s,await e(),t(),n())}}requestAnimationFrame(i)}}function k(a){const e=document.createElement("canvas");return e.height=a??window.innerHeight,e.width=e.height*(window.innerWidth/window.innerHeight),e.style.width="100vw",e.style.height="100vh",a&&(e.style.imageRendering="pixelated"),document.body.appendChild(e),e}class Y{canvas;main;functions;render;initalCustomUniforms={};setUniform;materialFuncs=[];usePrevFrameTex;useInterlacing;setDeviceTexure;texs;fps;showFps;rtCollision;rtUniform;camera={pos:{x:0,y:0,z:0},spherical:{radius:0,theta:0,phi:0},fov:90};constructor({main:e,functions:t,texs:n,usePrevFrameTex:r,useInterlacing:i,fps:s,height:o,showFps:c}={}){this.canvas=k(o),this.main=e,this.functions=t,this.texs=n??[],this.useInterlacing=i,this.usePrevFrameTex=this.useInterlacing||r,this.fps=s,this.showFps=c}registerMaterial(e){if(this.render)throw new Error("Render already built");const t=this.materialFuncs.length,n=new L({rt:this,material:e,index:t});return this.materialFuncs.push({sdFunc:e.sdFunc,lightFunc:e.lightFunc}),n}setTex(e,t){if(!this.setDeviceTexure)throw new Error("Render not built");this.setDeviceTexure(e,t)}registerUniform(e,t){if(this.render)throw new Error("Render already built");return this.initalCustomUniforms[e]=t??0,r=>{if(!this.setUniform)throw new Error("Render not built");this.setUniform(e,r),this.initalCustomUniforms[e]=r}}async build(){const{device:e,presentationFormat:t}=await N(),n=this.canvas.getContext("webgpu");if(!n)throw new Error("No context found");n.configure({device:e,format:t,alphaMode:"opaque",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.COPY_DST});const r=new F(e,{frame:0,time:0,aspectRatio:this.canvas.width/this.canvas.height,width:this.canvas.width,height:this.canvas.height,camPosX:this.camera.pos.x??0,camPosY:this.camera.pos.y??0,camPosZ:this.camera.pos.z??0,camSphericalR:this.camera.spherical.radius??0,camSphericalT:this.camera.spherical.theta??0,camSphericalP:this.camera.spherical.phi??0,camFov:this.camera.fov??90,...this.initalCustomUniforms});this.rtUniform=r;const i=new E(e,t,this.texs),s=[r.uniformBindGroupLayout,i.textureBindGroupLayout];this.render=await D({device:e,bindGroupLayouts:s,presentationFormat:t,context:n,canvas:this.canvas,main:this.main,materialFuncs:this.materialFuncs,functions:this.functions,rtUniform:r,rtTexture:i,usePrevFrameTex:this.usePrevFrameTex,useInterlacing:this.useInterlacing}),this.setUniform=(o,c)=>{r.set(o,c)},this.setDeviceTexure=(o,c)=>{i.setTexture(o,c)},this.rtCollision=new O({device:e,rtUniformKeys:r.getKeysSortedByOffset(),functions:this.functions,materialFuncs:this.materialFuncs,nTextures:this.texs.length,usePrevFrameTex:this.usePrevFrameTex,rtUniform:r,rtTexture:i})}checkCollision(e){if(!this.rtCollision||!this.rtUniform)throw new Error("Render not built");const t=this.rtUniform.get(`material${e}Px`),n=this.rtUniform.get(`material${e}Py`),r=this.rtUniform.get(`material${e}Pz`);return this.rtCollision.checkCollision(e,{x:t,y:n,z:r})}async start(e){if(await this.build(),e?.onBuild?.(),e?.once){this.render?.({camera:this.camera});return}new X({cb:e?.onFrame??(()=>{}),shoot:()=>{this.render?.({camera:this.camera})},fps:this.fps,showFps:this.showFps}).start()}}export{Y as R};
