...
`;function U({sdWGSL:a,materialSdFunctions:r}){function e(o,i){const l=`vec3<f32>(U.material${i}Px, U.material${i}Py, U.material${i}Pz)`,c=`vec3<f32>(U.material${i}Rx, U.material${i}Ry, U.material${i}Rz)`;return`
...
`);a=a.replace("// #SD-INDIVIDUAL-MATERIALS",t);function n(o){return`
...
`)}
      ...
      return material;
    `;return a=a.replace("// #SD-MATERIALS-FUNC",s),a}function I({materialSdFunctions:a,sdWGSL:r}){function e(n){return`dist = min(dist, sdMaterial${n}(pos));`}const t=`
      var dist: f32 = INF;
      var accDist: f32;
...
`)}
      ...
      return dist;`;return r=r.replace("// #MAP",t),r}function R({materialLightFunctions:a,sdWGSL:r}){function e(o,i){return o||(o=`
...
`);r=r.replace("// #LIGHT-INDIVIDUAL-MATERIALS",t);function n(o){return`
        if(materialIndex == ${o}) {
          finalColor = calcLightMaterial${o}(ro, rd, pos, normal, color);
        }`}const s=`
      var finalColor = vec4<f32>(0.0);
      ${a.map((o,i)=>n(i)).join(`
`)}
      return finalColor;`;return r=r.replace("// #LIGHT-MATERIALS-FUNC",s),r}function M(a){let r=_;const e=a.map(n=>n.sdFunc),t=a.map(n=>n.lightFunc);return r=U({sdWGSL:r,materialSdFunctions:e}),r=I({materialSdFunctions:e,sdWGSL:r}),r=R({sdWGSL:r,materialLightFunctions:t}),r}function P({functions:a,rtUniformKeys:r,materialFuncs:e,nTextures:t,usePrevFrameTex:n}){let s=z;if(s=s.replace("UNIFORMS: f32, // #UNIFORMS",r.map(o=>`${o}: f32,`).join(`
`)),t>0){let o=function(l){return`
        @group(1) @binding(${l}) var tex${l}: texture_2d<f32>;
        fn getTex${l}Sample(uv: vec2<f32>) -> vec4<f32> {
          return textureSample(tex${l}, u_sampler, uv);
        }`};const i=new Array(t).fill(0).map((l,c)=>o(c)).join(`
`);s=s.replace("// #GROUP-1-BINDING-X",i)}if(n&&(s=s.replace("// #GROUP-2-BINDING-X",`
      ...
      }`)),a&&(s=s.replace("// #FUNCTIONS",a)),e.length>0){const o=M(e);s=s.replace("// #RAY_MARCH_FUNCTIONS",o)}return s}function D({main:a,functions:r,rtUniformKeys:e,materialFuncs:t,nTextures:n,usePrevFrameTex:s,useInterlacing:o}){const i=P({functions:r,materialFuncs:t,nTextures:n,rtUniformKeys:e,usePrevFrameTex:s});t.length>0&&!a&&(a=`
  ...
  `;return o&&(l=l.replace("// #INTERLACING",`
      let _prev_pix_ = getPrevFrameTexSample(uv);
      if i32(U.height * uv.y) % 2 == i32(U.frame) % 2 {
        return _prev_pix_;
      }
    `)),l}class B{device;buffer;uniform={};uniformBindGroupLayout;uniformBindGroup;constructor(r,e){this.device=r;const t=Object.keys(e);this.buffer=r.createBuffer({size:t.length*4,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),t.forEach((n,s)=>{this.uniform[n]={value:e[n],offSet:s*4},this.set(n,e[n])}),this.uniformBindGroupLayout=r.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.FRAGMENT,sampler:{type:"non-filtering"}}]}),this.uniformBindGroup=this.device.createBindGroup({layout:this.uniformBindGroupLayout,entries:[{binding:0,resource:{buffer:this.getBuffer()}},{binding:1,resource:this.device.createSampler({addressModeU:"repeat",addressModeV:"repeat",magFilter:"nearest",minFilter:"nearest"})}]})}getBuffer(){return this.buffer}getKeysSortedByOffset(){return Object.keys(this.uniform).toSorted((r,e)=>this.uniform[r].offSet-this.uniform[e].offSet)}set(r,e){if(!this.uniform[r])throw new Error(`Uniform ${r} not found`);this.uniform[r].value=e,this.device.queue.writeBuffer(this.buffer,this.uniform[r].offSet,new Float32Array([e]))}get(r){if(!this.uniform[r])throw new Error(`Uniform ${r} not found`);return this.uniform[r].value}getCustomBindingsForCollisions(r){const e=this.device.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.COMPUTE,sampler:{type:"non-filtering"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}}]}),t=this.device.createBindGroup({layout:e,entries:[{binding:0,resource:{buffer:this.getBuffer()}},{binding:1,resource:this.device.createSampler({addressModeU:"repeat",addressModeV:"repeat",magFilter:"nearest",minFilter:"nearest"})},{binding:2,resource:{buffer:r}}]});return{uniformBindGroupLayout:e,uniformBindGroup:t}}}class F{texs;device;presentationFormat;textures;textureEntries;textureBindGroupLayout;constructor(r,e,t){this.texs=t,this.device=r,this.presentationFormat=e,this.textures=this.texs.map(({width:n,height:s})=>this.device.createTexture({size:[n,s,1],format:"rgba8unorm",usage:GPUTextureUsage.COPY_DST|GPUTextureUsage.TEXTURE_BINDING})),this.textureEntries=this.textures.map((n,s)=>({binding:s,resource:n.createView()})),this.textureBindGroupLayout=r.createBindGroupLayout({entries:t.map((n,s)=>({binding:s,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"unfilterable-float"}}))})}setTexture(r,e){const t=this.texs[r].width,n=this.texs[r].height,s=this.textures[r];this.device.queue.writeTexture({texture:s},e,{offset:0,bytesPerRow:t*4,rowsPerImage:n},[t,n,1])}getTextureEntries(){return this.textureEntries}length(){return this.texs.length}}class E{pingPongBindGroupLayout;bindGroupPingIn;bindGroupPongIn;currentReadTexture;currentReadTextureView;currentWriteTexture;currentWriteTextureView;currentBindGroupIn;offscreenTextureSize;constructor(r,e,t){this.offscreenTextureSize=[r.width,r.height,1];const n=e.createTexture({size:this.offscreenTextureSize,format:t,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),s=e.createTexture({size:this.offscreenTextureSize,format:t,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.TEXTURE_BINDING|GPUTextureUsage.COPY_SRC}),o=n.createView(),i=s.createView();this.pingPongBindGroupLayout=e.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.FRAGMENT,texture:{sampleType:"unfilterable-float"}}]}),this.bindGroupPingIn=e.createBindGroup({layout:this.pingPongBindGroupLayout,entries:[{binding:0,resource:n.createView()}]}),this.bindGroupPongIn=e.createBindGroup({layout:this.pingPongBindGroupLayout,entries:[{binding:0,resource:s.createView()}]}),this.currentReadTexture=n,this.currentReadTextureView=o,this.currentWriteTexture=s,this.currentWriteTextureView=i,this.currentBindGroupIn=this.bindGroupPingIn}preSubmit(){}swap(){const r=this.currentReadTexture,e=this.currentReadTextureView,t=this.currentBindGroupIn;this.currentReadTexture=this.currentWriteTexture,this.currentReadTextureView=this.currentWriteTextureView,this.currentBindGroupIn=t===this.bindGroupPingIn?this.bindGroupPongIn:this.bindGroupPingIn,this.currentWriteTexture=r,this.currentWriteTextureView=e}}async function A({device:a,presentationFormat:r,context:e,canvas:t,main:n,materialFuncs:s,functions:o,rtUniform:i,rtTexture:l,usePrevFrameTex:c,useInterlacing:p,bindGroupLayouts:d}){let u=null;c&&(u=new E(t,a,r),d.push(u.pingPongBindGroupLayout));const f=D({main:n,functions:o,materialFuncs:s,rtUniformKeys:i.getKeysSortedByOffset(),nTextures:l.length(),usePrevFrameTex:c,useInterlacing:p}),v=a.createRenderPipeline({layout:a.createPipelineLayout({bindGroupLayouts:d}),vertex:{module:a.createShaderModule({code:w}),entryPoint:"main"},fragment:{module:a.createShaderModule({code:f}),entryPoint:"main",targets:[{format:r}]},primitive:{topology:"triangle-strip",frontFace:"ccw",stripIndexFormat:"uint32"}});let y=0;const S=performance.now()/1e3;function T(m){const x=performance.now()/1e3-S;i.set("frame",y),i.set("time",x),i.set("aspectRatio",t.width/t.height),i.set("width",t.width),i.set("height",t.height),i.set("camPosX",m.pos.x),i.set("camPosY",m.pos.y),i.set("camPosZ",m.pos.z),i.set("camSphericalR",m.spherical.radius),i.set("camSphericalT",m.spherical.theta),i.set("camSphericalP",m.spherical.phi),i.set("camFov",m.fov)}const b=a.createBindGroup({layout:v.getBindGroupLayout(1),entries:l.getTextureEntries()});function G({camera:m}){T(m);const x={colorAttachments:[{view:u?u.currentWriteTextureView:e.getCurrentTexture().createView(),loadOp:"clear",clearValue:{r:0,g:0,b:0,a:1},storeOp:"store"}]},g=a.createCommandEncoder(),h=g.beginRenderPass(x);h.setPipeline(v),h.setBindGroup(0,i.uniformBindGroup),h.setBindGroup(1,b),u&&h.setBindGroup(2,u.currentBindGroupIn),h.draw(4,1,0,0),h.end(),u&&g.copyTextureToTexture({texture:u.currentWriteTexture},{texture:e.getCurrentTexture()},u.offscreenTextureSize),a.queue.submit([g.finish()]),u&&u.swap(),y++}return G}async function N(){const a=await navigator.gpu.requestAdapter({powerPreference:"high-performance"});if(!a)throw new Error("No adapter found");const r=a.features.has("texture-compression-bc");return r||console.warn("shader-f16 not available"),{device:await a.requestDevice({requiredFeatures:r?["shader-f16"]:[]}),presentationFormat:navigator.gpu.getPreferredCanvasFormat()}}class L{setPos;setRot;setColor;setCollisionGroup;checkForCollisions;collisionGroup=-1;index;rt;constructor({material:r,rt:e,index:t}){this.rt=e,r.pos||(r.pos={x:0,y:0,z:0}),r.color||(r.color={r:1,g:1,b:1}),r.rotation||(r.rotation={x:0,y:0,z:0});const n=e.registerUniform(`material${t}Px`,r.pos.x),s=e.registerUniform(`material${t}Py`,r.pos.y),o=e.registerUniform(`material${t}Pz`,r.pos.z);this.setPos=f=>{f.x!==void 0&&n(f.x),f.y!==void 0&&s(f.y),f.z!==void 0&&o(f.z)};const i=e.registerUniform(`material${t}Rx`,r.rotation.x),l=e.registerUniform(`material${t}Ry`,r.rotation.y),c=e.registerUniform(`material${t}Rz`,r.rotation.z);this.setRot=f=>{f.x!==void 0&&i(f.x),f.y!==void 0&&l(f.y),f.z!==void 0&&c(f.z)};const p=e.registerUniform(`material${t}Cr`,r.color.r),d=e.registerUniform(`material${t}Cg`,r.color.g),u=e.registerUniform(`material${t}Cb`,r.color.b);this.setColor=f=>{f.r!==void 0&&p(f.r),f.g!==void 0&&d(f.g),f.b!==void 0&&u(f.b)},r.enableCollisions&&(this.collisionGroup=t),this.setCollisionGroup=e.registerUniform(`material${t}CollisionGroup`,this.collisionGroup),this.index=t}checkCollision(){return this.rt.checkCollision(this.index)}getIndex(){return this.index}}function $({functions:a,rtUniformKeys:r,materialFuncs:e,nTextures:t,usePrevFrameTex:n}){const s=P({functions:a,materialFuncs:e,nTextures:t,rtUniformKeys:r,usePrevFrameTex:n}),o=e.map(c=>c.sdFunc);function i(c,p){return`
...
`)}
  ...
  `}class O{device;computePipeline;paramsBindGroup;paramsBuffer;uniformBindGroup;resultBufferSize;resultBuffer;rtTexture;constructor({device:r,functions:e,rtUniformKeys:t,materialFuncs:n,nTextures:s,usePrevFrameTex:o,rtUniform:i,rtTexture:l}){this.device=r,this.rtTexture=l;const c=5;this.resultBufferSize=Float32Array.BYTES_PER_ELEMENT*c,this.resultBuffer=r.createBuffer({size:this.resultBufferSize,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC});const{uniformBindGroupLayout:p,uniformBindGroup:d}=i.getCustomBindingsForCollisions(this.resultBuffer);this.uniformBindGroup=d,this.paramsBuffer=r.createBuffer({size:32,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});const u=r.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}}]});this.paramsBindGroup=r.createBindGroup({layout:u,entries:[{binding:0,resource:{buffer:this.paramsBuffer}}]});const f=[p,this.rtTexture.textureBindGroupLayout,u],v=$({functions:e,rtUniformKeys:t,materialFuncs:n,nTextures:s,usePrevFrameTex:o});this.computePipeline=r.createComputePipeline({layout:r.createPipelineLayout({bindGroupLayouts:f}),compute:{module:r.createShaderModule({code:v}),entryPoint:"main"}})}async checkCollision(r,e){this.device.queue.writeBuffer(this.paramsBuffer,0,new Int32Array([r])),this.device.queue.writeBuffer(this.paramsBuffer,16,new Float32Array([e.x,e.y,e.z]));const t=this.device.createCommandEncoder(),n=t.beginComputePass();n.setPipeline(this.computePipeline),n.setBindGroup(0,this.uniformBindGroup);const s=this.device.createBindGroup({layout:this.computePipeline.getBindGroupLayout(1),entries:this.rtTexture.getTextureEntries()});n.setBindGroup(1,s),n.setBindGroup(2,this.paramsBindGroup),n.dispatchWorkgroups(1),n.end();const o=this.device.createBuffer({size:this.resultBufferSize,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ});t.copyBufferToBuffer(this.resultBuffer,0,o,0,this.resultBufferSize),this.device.queue.submit([t.finish()]),await o.mapAsync(GPUMapMode.READ);const i=new Float32Array(o.getMappedRange());try{const l=i[0],c=i[1],p={x:i[2],y:i[3],z:i[4]};return{materialIndex:l,collisionDist:c,collisionNormal:p}}finally{o.unmap()}}}class k{shoot;fps;fpsCounter=0;showFps;cb;constructor({cb:r,shoot:e,fps:t,showFps:n}){this.shoot=e,this.cb=r,this.fps=t,this.showFps=n}startFpsCounter(){const r=document.createElement("div");r.setAttribute("style",` position: absolute;
        top: 0;
        left: 0;
        color: green;
        background: black;
        ...
        pointer-events: none;`),document.body.appendChild(r),setInterval(()=>{r.innerText=`FPS: ${this.fpsCounter}`,this.fpsCounter=0},1e3)}start(){this.showFps&&this.startFpsCounter();const r=this.cb,e=this.shoot,t=()=>{this.fpsCounter++},n=this.fps;let s=async()=>{requestAnimationFrame(s),await r(),e(),t()};if(n!==void 0){const o=1e3/n;let i=0;s=async l=>{requestAnimationFrame(s);const c=l-i;c>o&&(i=l-c%o,await r(),e(),t())}}requestAnimationFrame(s)}}function X(a){const r=document.createElement("canvas");return r.height=a??window.innerHeight,r.width=r.height*(window.innerWidth/window.innerHeight),r.style.width="100vw",r.style.height="100vh",a&&(r.style.imageRendering="pixelated"),document.body.appendChild(r),r}class Y{canvas;main;functions;render;initalCustomUniforms={};setUniform;materialFuncs=[];usePrevFrameTex;useInterlacing;setDeviceTexure;texs;fps;showFps;rtCollision;rtUniform;camera={pos:{x:0,y:0,z:0},spherical:{radius:0,theta:0,phi:0},fov:90};constructor({main:r,functions:e,texs:t,usePrevFrameTex:n,useInterlacing:s,fps:o,height:i,showFps:l}={}){this.canvas=X(i),this.main=r,this.functions=e,this.texs=t??[],this.useInterlacing=s,this.usePrevFrameTex=this.useInterlacing||n,this.fps=o,this.showFps=l}registerMaterial(r){if(this.render)throw new Error("Render already built");const e=this.materialFuncs.length,t=new L({rt:this,material:r,index:e});return this.materialFuncs.push({sdFunc:r.sdFunc,lightFunc:r.lightFunc}),t}setTex(r,e){if(!this.setDeviceTexure)throw new Error("Render not built");this.setDeviceTexure(r,e)}registerUniform(r,e){if(this.render)throw new Error("Render already built");return this.initalCustomUniforms[r]=e??0,n=>{if(!this.setUniform)throw new Error("Render not built");this.setUniform(r,n),this.initalCustomUniforms[r]=n}}async build(){const{device:r,presentationFormat:e}=await N(),t=this.canvas.getContext("webgpu");if(!t)throw new Error("No context found");t.configure({device:r,format:e,alphaMode:"opaque",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.COPY_DST});const n=new B(r,{frame:0,time:0,aspectRatio:this.canvas.width/this.canvas.height,width:this.canvas.width,height:this.canvas.height,camPosX:this.camera.pos.x??0,camPosY:this.camera.pos.y??0,camPosZ:this.camera.pos.z??0,camSphericalR:this.camera.spherical.radius??0,camSphericalT:this.camera.spherical.theta??0,camSphericalP:this.camera.spherical.phi??0,camFov:this.camera.fov??90,...this.initalCustomUniforms});this.rtUniform=n;const s=new F(r,e,this.texs),o=[n.uniformBindGroupLayout,s.textureBindGroupLayout];this.render=await A({device:r,bindGroupLayouts:o,presentationFormat:e,context:t,canvas:this.canvas,main:this.main,materialFuncs:this.materialFuncs,functions:this.functions,rtUniform:n,rtTexture:s,usePrevFrameTex:this.usePrevFrameTex,useInterlacing:this.useInterlacing}),this.setUniform=(i,l)=>{n.set(i,l)},this.setDeviceTexure=(i,l)=>{s.setTexture(i,l)},this.rtCollision=new O({device:r,rtUniformKeys:n.getKeysSortedByOffset(),functions:this.functions,materialFuncs:this.materialFuncs,nTextures:this.texs.length,usePrevFrameTex:this.usePrevFrameTex,rtUniform:n,rtTexture:s})}checkCollision(r){if(!this.rtCollision||!this.rtUniform)throw new Error("Render not built");const e=this.rtUniform.get(`material${r}Px`),t=this.rtUniform.get(`material${r}Py`),n=this.rtUniform.get(`material${r}Pz`);return this.rtCollision.checkCollision(r,{x:e,y:t,z:n})}async start(r){if(await this.build(),r?.onBuild?.(),r?.once){this.render?.({camera:this.camera});return}new k({cb:r?.onFrame??(()=>{}),shoot:()=>{this.render?.({camera:this.camera})},fps:this.fps,showFps:this.showFps}).start()}}export{Y as R};
